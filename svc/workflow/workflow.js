...
export const getWorkflowDefinition = async (code, interactionType, intxnCatType) => {
  try {
    ...
    getWorkflow.forEach(e => {
      if (intxnCatType && !code) {
        if (e.productType === intxnCatType) {
          workFlow.push(e)
        }
      } else {
...
export const setWorkflow = async (code, interactionType, intxnId) => {
  try {
    ...
    logger.debug('setWorkflow :- Sucessfully set workflow', data)
...
async function findEntites (data) {
  ...
  console.log('findEntites1', data)
  const response = []
  console.log(data.length)
  for (const d of data) {
    console.log('findEntites1', "select s.code, s.description from business_entity s where s.code in ('" + d.status.join("','") + "')")
    const unit = {
